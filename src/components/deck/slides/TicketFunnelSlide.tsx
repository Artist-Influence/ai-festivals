  ...
  return (
    <div className="w-full min-h-dvh md:h-full bg-background relative overflow-hidden p-5 md:p-16 flex flex-col justify-start md:justify-center">
      ...
      <div className="relative z-10 w-full max-w-[1750px] mx-auto">
        <p className="t-eyebrow text-xs md:text-sm mb-2 md:mb-4">Ticket Sales Funnel</p>
            ...
            {stages.map((i) => (
              <GlassPanel key={i} variant="subtle" className="p-3 md:p-6">
                <div className="flex flex-col md:flex-row md:items-start gap-1 md:gap-6">
                  <span className="t-eyebrow text-sm md:text-xl md:w-40 md:shrink-0">{t(`funnel.stage.${i}.label`)}</span>
        ...
        <GlassPanel variant="subtle" className="mt-4 md:mt-8 p-3 md:p-5">
          <p className="text-sm md:text-lg text-muted-foreground text-center">{t('funnel.footer')}</p>
        </GlassPanel>
      </div>
    </div>
  );
